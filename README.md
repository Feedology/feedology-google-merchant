...
## Installation from GitHub Package Registry

### Setup Authentication

1. Create a GitHub Personal Access Token with `read:packages` permission at [GitHub Settings > Tokens](https://github.com/settings/tokens)

2. Create or update `.npmrc` file in your project root:

```ini
@Feedology:registry=https://npm.pkg.github.com
//npm.pkg.github.com/:_authToken=YOUR_GITHUB_TOKEN
```

Or add to your `~/.npmrc` for global use:
...
```bash
yarn add @Feedology/google-merchant
# or
npm install @Feedology/google-merchant
```

**Repository:** [Feedology/feedology-google-merchant](https://github.com/Feedology/feedology-google-merchant)

## Publishing to GitHub Package Registry

### Prerequisites

1. Repository is configured: `git@github.com:Feedology/feedology-google-merchant.git`
2. Create GitHub Personal Access Token with `write:packages` permission
3. Configure authentication (see `.npmrc.example`):
   ```bash
...
A workflow file (`.github/workflows/publish.yml`) is included for automated publishing on releases. The workflow will automatically publish when you create a new release on GitHub.
