...
import type {
    GoogleMerchantProductTransformInput,
    TransformerShop,
    TransformerFeed,
    TransformerProduct,
 ...
 */
function encryptToString(data: Record<string, unknown>): string {
...
export class GoogleMerchantProductTransformer {
    ...
    transform(input: GoogleMerchantProductTransformInput): ProductInput {
        ...
        if (customLabels && customLabels.length > 0) {
            if (customLabels[0]) attributes.customLabel0 = customLabels[0];
            if (customLabels[1]) attributes.customLabel1 = customLabels[1];
            if (customLabels[2]) attributes.customLabel2 = customLabels[2];
            if (customLabels[3]) attributes.customLabel3 = customLabels[3];
            if (customLabels[4]) attributes.customLabel4 = customLabels[4];
        }
        ...
        if (apparelProductDetails) {
            if (apparelProductDetails.gender) attributes.gender = apparelProductDetails.gender as string;
            if (apparelProductDetails.age_group) attributes.ageGroup = apparelProductDetails.age_group as string;
            if (apparelProductDetails.size) attributes.size = apparelProductDetails.size as string;
            if (apparelProductDetails.size_type) attributes.sizeTypes = apparelProductDetails.size_type as string[];
            if (apparelProductDetails.size_system) attributes.sizeSystem = apparelProductDetails.size_system as string;
            if (apparelProductDetails.color) attributes.color = apparelProductDetails.color as string;
            if (apparelProductDetails.material) attributes.material = apparelProductDetails.material as string;
            if (apparelProductDetails.pattern) attributes.pattern = apparelProductDetails.pattern as string;
        }

        // Additional Details
        const additionalDetails = this.getAdditionalDetails(feedProductVariant);
        if (additionalDetails) {
            if (additionalDetails.adult) attributes.adult = additionalDetails.adult as boolean;
            if (additionalDetails.is_bundle) attributes.isBundle = additionalDetails.is_bundle as boolean;
            if (additionalDetails.multipack) attributes.multipack = additionalDetails.multipack as string;
            if (additionalDetails.energy_efficiency_class) attributes.energyEfficiencyClass = additionalDetails.energy_efficiency_class as string;
            if (additionalDetails.min_energy_efficiency_class) attributes.minEnergyEfficiencyClass = additionalDetails.min_energy_efficiency_class as string;
            if (additionalDetails.max_energy_efficiency_class) attributes.maxEnergyEfficiencyClass = additionalDetails.max_energy_efficiency_class as string;
            if (additionalDetails.product_highlights) attributes.productHighlights = additionalDetails.product_highlights as string[];
            if (additionalDetails.certifications) attributes.certifications = additionalDetails.certifications as Array<{
        ...
        if (shippingAndReturns) {
            if (shippingAndReturns.shipping_label) attributes.shippingLabel = shippingAndReturns.shipping_label as string;
            if (shippingAndReturns.shipping_weight_value) attributes.shippingWeight = {
                value: shippingAndReturns.shipping_weight_value as number,
                unit: shippingAndReturns.shipping_weight_unit as string,
            };
            if (shippingAndReturns.shipping_length_value) attributes.shippingLength = {
                value: shippingAndReturns.shipping_length_value as number,
                unit: shippingAndReturns.shipping_length_unit as string,
            };
            if (shippingAndReturns.shipping_width_value) attributes.shippingWidth = {
                value: shippingAndReturns.shipping_width_value as number,
                unit: shippingAndReturns.shipping_width_unit as string,
            };
            if (shippingAndReturns.shipping_height_value) attributes.shippingHeight = {
                value: shippingAndReturns.shipping_height_value as number,
                unit: shippingAndReturns.shipping_height_unit as string,
            };
            if (shippingAndReturns.transit_time_label) attributes.transitTimeLabel = shippingAndReturns.transit_time_label as string;
            if (shippingAndReturns.minimun_handling_time) attributes.minHandlingTime = shippingAndReturns.minimun_handling_time as string;
            if (shippingAndReturns.maximum_handling_time) attributes.maxHandlingTime = shippingAndReturns.maximum_handling_time as string;
        }
        ...
        if (additionalProductAttributes && Object.keys(additionalProductAttributes).length > 0) {
            for (const [attribute, value] of Object.entries(additionalProductAttributes)) {
                ...
                const finalValue = Array.isArray(value) ? value.join(',') : (value as string);
                customAttributes.push({
                    name: attribute,
                    value: finalValue,
                })
     ...
     */
    private getItemGroupId(feedProductVariant: TransformerFeedProductVariant): string {
        let itemGroupId = feedProductVariant.product_id;
        const fieldMapping = feedProductVariant.field_mapping as Record<string, unknown> | null | undefined;
        if (fieldMapping?.product_details) {
            const productDetails = fieldMapping.product_details as Record<string, unknown>;
            ...
            if (productDetails.item_group_id) {
                itemGroupId = (productDetails.item_group_id as string)
     ...
     */
    private getProductId(feed: TransformerFeed, feedProductVariant: TransformerFeedProductVariant): string {
     ...
     */
    private getTitle(feed: TransformerFeed, product: TransformerProduct, variant: TransformerProductVariant, feedProductVariant: TransformerFeedProductVariant): string {
        let title = feed.product_settings?.product_title || '';
        if (feedProductVariant.field_mapping) {
            const fieldMapping = feedProductVariant.field_mapping as Record<string, unknown>;
            if (fieldMapping.product_details) {
                const productDetails = fieldMapping.product_details as Record<string, unknown>;
                ...
                if (productDetails.product_title) {
                    title = (productDetails.product_title as string);
                }
            }
        }
        return title
            ...
            .replace('{{seo_title}}', product?.seo && typeof (product.seo as unknown as Record<string, unknown>) === 'object' && (product.seo as unknown as Record<string, unknown>) !== null
                ? ((product.seo as unknown as Record<string, unknown>).title as string) || ''
                : '')
     ...
     */
    private getDescription(feed: TransformerFeed, product: TransformerProduct, variant: TransformerProductVariant, feedProductVariant: TransformerFeedProductVariant): string {
        ...
        return description
            ...
            .replace('{{seo_description}}', product?.seo && typeof (product.seo as unknown as Record<string, unknown>) === 'object' && (product.seo as unknown as Record<string, unknown>) !== null
                ? ((product.seo as unknown as Record<string, unknown>).description as string) || ''
                : '');
    }
     ...
     */
    private getAccountId(feed: TransformerFeed): string {
     ...
     */
    private getPriceAmountMicros(product: TransformerProduct, variant: TransformerProductVariant, feedProductVariant: TransformerFeedProductVariant): string {
        ...
        if (feedProductVariant.field_mapping) {
            ...
            if (fieldMapping.product_details) {
                const productDetails = fieldMapping.product_details as Record<string, unknown>;
                if (productDetails.price_condition_availability && typeof (productDetails.price_condition_availability as Record<string, unknown>) === 'object' && (productDetails.price_condition_availability as Record<string, unknown>) !== null && (productDetails.price_condition_availability as Record<string, unknown>).price) {
                    priceType = (productDetails.price_condition_availability as Record<string, unknown>).price as string;
                }
            }
        }
     ...
     */
    private getBrand(feed: TransformerFeed, shop: TransformerShop, product: TransformerProduct, feedProductVariant: TransformerFeedProductVariant): string | null {
        let brand = null;
        let brandSource: string | null = feed.product_settings?.brand_submission || null;
        if (feedProductVariant.field_mapping) {
            const fieldMapping = feedProductVariant.field_mapping as Record<string, unknown>;
            if (fieldMapping.product_details) {
                const productDetails = fieldMapping.product_details as Record<string, unknown>;
                if (productDetails.brand) {
                    brandSource = (productDetails.brand as string) as BrandSubmissionType;
                }
            }
        }
     ...
     */
    private getIdentifierExists(feedProductVariant: TransformerFeedProductVariant): boolean {
        ...
        if (feedProductVariant.field_mapping) {
            ...
            if (fieldMapping.product_details) {
                ...
                if (productDetails.product_identifier) {
                    identifierExists = productDetails.product_identifier as boolean;
                }
            }
        }
     ...
     */
    private replaceTemplateVariables(template: string, variant: TransformerProductVariant, feedProductVariant: TransformerFeedProductVariant): string {
        return template
            .replace('{{barcode}}', variant.barcode || '')
            .replace('{{sku}}', variant.sku || '')
            .replace('{{product_id}}', feedProductVariant.product_id)
     ...
     */
    private getGtinsMpn(feed: TransformerFeed, variant: TransformerProductVariant, feedProductVariant: TransformerFeedProductVariant): { gtins?: string[] | null, mpn?: string | null } | null {
        ...
        const fieldMapping = feedProductVariant.field_mapping as Record<string, unknown> | null | undefined;
        const productDetails = fieldMapping?.product_details as Record<string, unknown> | undefined;

        // Process GTIN template if present
        let gtinTemplate: string | undefined;
        if (productDetails?.gtin) {
            gtinTemplate = this.replaceTemplateVariables(productDetails.gtin as string, variant, feedProductVariant);
        }

        // Process MPN template if present
        let mpnTemplate: string | undefined;
        if (productDetails?.mpn) {
            mpnTemplate = this.replaceTemplateVariables(productDetails.mpn as string, variant, feedProductVariant);
        }
     ...
     */
    private getLink(shop: TransformerShop, feed: TransformerFeed, product: TransformerProduct, variant: TransformerProductVariant, feedProductVariant: TransformerFeedProductVariant): string {
        let link = shop.domain + '/products/' + product.handle;

        let linkType: string = PRODUCT_URL_SOURCE_TYPES.PRODUCT_URL;
        if (feedProductVariant.field_mapping) {
            const fieldMapping = feedProductVariant.field_mapping as Record<string, unknown>;
            if (fieldMapping.links) {
                const links = fieldMapping.links as Record<string, unknown>;
                if (links.product_url) {
                    linkType = links.product_url as string;
                }
            }
        }
     ...
     */
    private addUtmParameters(link: string, feed: TransformerFeed, feedProductVariant: TransformerFeedProductVariant): string {
        const utmParameters = [];
        if (feed.tracking) {
     ...
     */
    private getCanonicalLink(shop: TransformerShop, feed: TransformerFeed, product: TransformerProduct): string {
     ...
     */
    private getImageLink(mainImage: string | null, feedProductVariant: TransformerFeedProductVariant): string | null {
        let imageLink = mainImage;
        if (feedProductVariant.field_mapping) {
            const fieldMapping = feedProductVariant.field_mapping as Record<string, unknown>;
            if (fieldMapping.product_images) {
                const productImages = fieldMapping.product_images as Record<string, unknown>;
                if (productImages.main_image) {
                    imageLink = productImages.main_image as string;
                }
            }
        }
        return imageLink;
     ...
     */
    private getAdditionalImageLinks(feedProductVariant: TransformerFeedProductVariant): string[] {
        ...
        if (feedProductVariant.field_mapping) {
            ...
            if (fieldMapping.product_images) {
                ...
                if (productImages.additional_images) {
                    additionalImageLinks = productImages.additional_images as string[];
                }
            }
        }
    ...
    private getPrice(variant: TransformerProductVariant, feedProductVariant: TransformerFeedProductVariant): string | null {
        let price: string | null = null;
        let priceType: string = PRICE_TYPES.PRICE;
        if (feedProductVariant.field_mapping) {
            const fieldMapping = feedProductVariant.field_mapping as Record<string, unknown>;
            if (fieldMapping.price_condition_availability) {
                const priceConditionAvailability = fieldMapping.price_condition_availability as Record<string, unknown>;
                if (priceConditionAvailability.price) {
                    priceType = priceConditionAvailability.price as string;
                }
            }
        }
     ...
     */
    private getSalePrice(feed: TransformerFeed, variant: TransformerProductVariant, feedProductVariant: TransformerFeedProductVariant): {
        salePrice?: { amountMicros: string; currencyCode: string; };
        salePriceEffectiveDate?: { startTime?: string; endTime?: string; };
    } | null {
        let enableSalePrice = feed.product_settings?.enable_sale_price as boolean | undefined ?? false;
        if (!enableSalePrice) {
            return null;
        }
        ...
        enableSalePrice = priceConditionAvailability.enable_sale_price as boolean | undefined ?? false;
        if (!enableSalePrice) {
            return null;
        }

        const salePrice: { amountMicros: string; currencyCode: string; } = { amountMicros: '', currencyCode: '' };
        const salePriceEffectiveDate: { startTime?: string; endTime?: string; } = {};

        // Get sale price type
        const salePriceType = priceConditionAvailability.sale_price as string | undefined;
        if (salePriceType) {
        ...
        // Get sale dates
        const saleStartDate = priceConditionAvailability.sale_start_date as string | undefined;
        if (saleStartDate) {
            salePriceEffectiveDate.startTime = saleStartDate;
        }
        const saleEndDate = priceConditionAvailability.sale_end_date as string | undefined;
        if (saleEndDate) {
     ...
     */
    private getAutoPricingMinPrice(feedProductVariant: TransformerFeedProductVariant): string | null {
        let autoPricingMinPrice: string | null = null;
        if (feedProductVariant.field_mapping) {
            const fieldMapping = feedProductVariant.field_mapping as Record<string, unknown>;
            if (fieldMapping.price_condition_availability) {
                const priceConditionAvailability = fieldMapping.price_condition_availability as Record<string, unknown>;
                if (priceConditionAvailability.auto_pricing_min_price) {
                    autoPricingMinPrice = priceConditionAvailability.auto_pricing_min_price as string;
                }
            }
        }
     ...
     */
    private getMaximumRetailPrice(feedProductVariant: TransformerFeedProductVariant): string | null {
        let maximumRetailPrice: string | null = null;
        if (feedProductVariant.field_mapping) {
            const fieldMapping = feedProductVariant.field_mapping as Record<string, unknown>;
            if (fieldMapping.price_condition_availability) {
                const priceConditionAvailability = fieldMapping.price_condition_availability as Record<string, unknown>;
                if (priceConditionAvailability.maximum_retail_price) {
                    maximumRetailPrice = priceConditionAvailability.maximum_retail_price as string;
                }
            }
        }
     ...
     */
    private getCondition(feedProductVariant: TransformerFeedProductVariant): string | null {
        ...
        if (feedProductVariant.field_mapping) {
            ...
            if (fieldMapping.price_condition_availability) {
                ...
                if (priceConditionAvailability.condition) {
                    condition = priceConditionAvailability.condition as string;
                }
            }
        }
        return condition;
     ...
     */
    private getAvailability(feedProductVariant: TransformerFeedProductVariant): string | null {
        ...
        if (feedProductVariant.field_mapping) {
            ...
            if (fieldMapping.price_condition_availability) {
                ...
                if (priceConditionAvailability.availability) {
                    availability = priceConditionAvailability.availability as string;
                }
            }
        }
        return availability;
     ...
     */
    private getProductType(product: TransformerProduct, feedProductVariant: TransformerFeedProductVariant): string[] | null {
        let productTypes: string[] | null = null;

        let productTypeField: string = PRODUCT_TYPE_FIELDS.PRODUCT_TYPE;
        if (feedProductVariant.field_mapping) {
            const fieldMapping = feedProductVariant.field_mapping as Record<string, unknown>;
            if (fieldMapping.price_condition_availability) {
                const priceConditionAvailability = fieldMapping.price_condition_availability as Record<string, unknown>;
                if (priceConditionAvailability.product_type) {
                    productTypeField = priceConditionAvailability.product_type as keyof typeof PRODUCT_TYPE_FIELDS;
                }
            }
        }
     ...
     */
    private getGoogleProductCategory(feed: TransformerFeed, feedProductVariant: TransformerFeedProductVariant): string | null {
        let googleProductCategory: string | null = feed.metadata?.google_merchant_center?.product_category_id?.toString() || null;
        if (feedProductVariant.field_mapping) {
            const fieldMapping = feedProductVariant.field_mapping as Record<string, unknown>;
            if (fieldMapping.price_condition_availability) {
                const priceConditionAvailability = fieldMapping.price_condition_availability as Record<string, unknown>;
                if (priceConditionAvailability.google_product_category) {
                    googleProductCategory = priceConditionAvailability.google_product_category as string;
                }
            }
        }
     ...
     */
    private getCustomLabels(feedProductVariant: TransformerFeedProductVariant): string[] | null {
        const customLabels: string[] = [];
        if (feedProductVariant.field_mapping) {
            const fieldMapping = feedProductVariant.field_mapping as Record<string, unknown>;
            if (fieldMapping.labels) {
                const labels = fieldMapping.labels as Record<string, unknown>;
                if (labels.custom_label_0) {
                    customLabels.push(labels.custom_label_0 as string);
                }
                if (labels.custom_label_1) {
                    customLabels.push(labels.custom_label_1 as string);
                }
                if (labels.custom_label_2) {
                    customLabels.push(labels.custom_label_2 as string);
                }
                if (labels.custom_label_3) {
                    customLabels.push(labels.custom_label_3 as string);
                }
                if (labels.custom_label_4) {
                    customLabels.push(labels.custom_label_4 as string);
                }
            }
        }
        return customLabels;
     ...
     */
    private getApparelProductDetails(feedProductVariant: TransformerFeedProductVariant): Record<string, unknown> | null {
        let apparelProductDetails: Record<string, unknown> | null = null;
        if (feedProductVariant.field_mapping) {
            const fieldMapping = feedProductVariant.field_mapping as Record<string, unknown>;
            if (fieldMapping.apparel_product_details) {
                apparelProductDetails = fieldMapping.apparel_product_details as Record<string, unknown>;
                if (apparelProductDetails.gender) {
                    apparelProductDetails.gender = apparelProductDetails.gender as string;
                }
                if (apparelProductDetails.age_group) {
                    apparelProductDetails.age_group = apparelProductDetails.age_group as string;
                }
                if (apparelProductDetails.size) {
                    apparelProductDetails.size = apparelProductDetails.size as string;
                }
                if (apparelProductDetails.size_type) {
                    apparelProductDetails.size_type = apparelProductDetails.size_type as string;
                }
                if (apparelProductDetails.size_system) {
                    apparelProductDetails.size_system = apparelProductDetails.size_system as string;
                }
                if (apparelProductDetails.color) {
                    apparelProductDetails.color = apparelProductDetails.color as string;
                }
                if (apparelProductDetails.material) {
                    apparelProductDetails.material = apparelProductDetails.material as string;
                }
                if (apparelProductDetails.pattern) {
                    apparelProductDetails.pattern = apparelProductDetails.pattern as string;
                }
            }
        }
     ...
     */
    private getAdditionalDetails(feedProductVariant: TransformerFeedProductVariant): Record<string, unknown> | null {
        let additionalDetails: Record<string, unknown> | null = null;
        if (feedProductVariant.field_mapping) {
            const fieldMapping = feedProductVariant.field_mapping as Record<string, unknown>;
            if (fieldMapping.additional_details) {
                additionalDetails = fieldMapping.additional_details as Record<string, unknown>;
                if (additionalDetails.adult) {
                    additionalDetails.adult = additionalDetails.adult as boolean;
                }
                if (additionalDetails.is_bundle) {
                    additionalDetails.is_bundle = additionalDetails.is_bundle as boolean;
                }
                if (additionalDetails.multipack) {
                    additionalDetails.multipack = additionalDetails.multipack as string;
                }
                if (additionalDetails.energy_efficiency_class) {
                    additionalDetails.energy_efficiency_class = additionalDetails.energy_efficiency_class as string;
                }
                if (additionalDetails.min_energy_efficiency_class) {
                    additionalDetails.min_energy_efficiency_class = additionalDetails.min_energy_efficiency_class as string;
                }
                if (additionalDetails.max_energy_efficiency_class) {
                    additionalDetails.max_energy_efficiency_class = additionalDetails.max_energy_efficiency_class as string;
                }
                if (additionalDetails.product_highlights) {
                    additionalDetails.product_highlights = additionalDetails.product_highlights as string[];
                }
                if (additionalDetails.certifications) {
                    ...
                    for (const certification of additionalDetails.certifications as Array<{
                        authority?: string;
                        name?: string;
                        code?: string;
                        value?: string;
     ...
     */
    private getShippingAndReturns(feedProductVariant: TransformerFeedProductVariant): Record<string, unknown> | null {
        let shippingAndReturns: Record<string, unknown> | null = null;
        if (feedProductVariant.field_mapping) {
            const fieldMapping = feedProductVariant.field_mapping as Record<string, unknown>;
            if (fieldMapping.shipping_and_returns) {
                shippingAndReturns = fieldMapping.shipping_and_returns as Record<string, unknown>;
                if (shippingAndReturns.shipping_label) {
                    shippingAndReturns.shipping_label = shippingAndReturns.shipping_label as string;
                }
                if (shippingAndReturns.shipping_weight_value) {
                    shippingAndReturns.shipping_weight_value = shippingAndReturns.shipping_weight_value as number;
                }
                if (shippingAndReturns.shipping_weight_unit) {
                    shippingAndReturns.shipping_weight_unit = shippingAndReturns.shipping_weight_unit as string;
                }
                if (shippingAndReturns.shipping_length_value) {
                    shippingAndReturns.shipping_length_value = shippingAndReturns.shipping_length_value as number;
                }
                if (shippingAndReturns.shipping_length_unit) {
                    shippingAndReturns.shipping_length_unit = shippingAndReturns.shipping_length_unit as string;
                }
                if (shippingAndReturns.shipping_width_value) {
                    shippingAndReturns.shipping_width_value = shippingAndReturns.shipping_width_value as number;
                }
                if (shippingAndReturns.shipping_width_unit) {
                    shippingAndReturns.shipping_width_unit = shippingAndReturns.shipping_width_unit as string;
                }
                if (shippingAndReturns.shipping_height_value) {
                    shippingAndReturns.shipping_height_value = shippingAndReturns.shipping_height_value as number;
                }
                if (shippingAndReturns.shipping_height_unit) {
                    shippingAndReturns.shipping_height_unit = shippingAndReturns.shipping_height_unit as string;
                }
                if (shippingAndReturns.transit_time_label) {
                    shippingAndReturns.transit_time_label = shippingAndReturns.transit_time_label as string;
                }
                if (shippingAndReturns.minimun_handling_time) {
                    shippingAndReturns.minimun_handling_time = shippingAndReturns.minimun_handling_time as string;
                }
                if (shippingAndReturns.maximum_handling_time) {
                    shippingAndReturns.maximum_handling_time = shippingAndReturns.maximum_handling_time as string;
                }
            }
        }
     ...
     */
    private getReturnPolicyLabels(feedProductVariant: TransformerFeedProductVariant): string[] | null {
        let returnPolicyLabels: string[] | null = null;
        if (feedProductVariant.field_mapping) {
            const fieldMapping = feedProductVariant.field_mapping as Record<string, unknown>;
            if (fieldMapping.shipping_and_returns) {
                const shippingAndReturns = fieldMapping.shipping_and_returns as Record<string, unknown>;
                if (shippingAndReturns.return_policy_labels) {
                    returnPolicyLabels = shippingAndReturns.return_policy_labels as string[];
                }
            }
        }
     ...
     */
    private getAdditionalProductAttributes(feedProductVariant: TransformerFeedProductVariant): Record<string, unknown> | null {
        let additionalProductAttributes: Record<string, unknown> | null = null;
        if (feedProductVariant.field_mapping) {
            const fieldMapping = feedProductVariant.field_mapping as Record<string, unknown>;
            if (fieldMapping.additional_product_attributes) {
                additionalProductAttributes = fieldMapping.additional_product_attributes as Record<string, unknown>;
            }
        }
